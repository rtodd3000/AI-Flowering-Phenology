...
# 3-level intensity scale
INTENSITY_CLASSES = [
    "0 - No Flowers",
    "1 - Partially Flowering",
    "2 - Full Bloom"
]
...
def append_to_csv(df, image_name, site, flower_type, intensity):
    ...
    Intensity is stored as raw integer (0, 1, 2).
    """
    # Extract raw intensity number from label string e.g. "1 - Partially Flowering" -> 1
...
def predict_single(img_path):
    ...
    site = args.site if args.site else os.path.basename(os.path.dirname(img_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--site", type=str, help="optional, inferred from folder if not provided")
